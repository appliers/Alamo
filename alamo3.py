...
if optionInput == "1":
    ...
    urlInput = input(f"[{Fore.RED}Alamo{Style.RESET_ALL}] {Fore.CYAN}Scam URL?: {Style.RESET_ALL}")

    if urlInput.lower() == f"https://{urlInput}":
        urlInput = url
    else:
        url = f"https://{urlInput}"
    ...
    try:
        usernameInput = input(f"[{Fore.RED}Alamo{Style.RESET_ALL}] {Fore.CYAN}Login Form Data?: {Style.RESET_ALL}")
        if usernameInput == "":
    ...
    try:
        passwordInput = input(f"[{Fore.RED}Alamo{Style.RESET_ALL}] {Fore.CYAN}Password Form Data?: {Style.RESET_ALL}")
        if passwordInput == "":
            print(f"[{Fore.RED}Alamo{Style.RESET_ALL}] {Fore.CYAN}You didn't enter anything! Please try again.{Style.RESET_ALL}")
            exit()
    except ValueError:
