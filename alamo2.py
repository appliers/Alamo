    ...
    if proxyInput == "yes":
        ...
        #	})
            ...
            try:
                ...
                print(f"[{Fore.RED}Alamo{Style.RESET_ALL}] {Fore.CYAN}You didn't enter anything for either Login Form Data or Password Form Data")
            except:
                print(f"[{Fore.RED}Alamo{Style.RESET_ALL}] {Fore.CYAN}Failed to connect to the proxies! Please try again.{Style.RESET_ALL}")
                exit();
# raise ProxyError(f'\n{style.BOLD}[{Fore.RED}Error{Style.RESET_ALL}] {Fore.GREEN}"{request2Input}" Failed to connect to the proxies. Please try again.')
        ...
        try:
            ...
            print(f"[{Fore.RED}Alamo{Style.RESET_ALL}] {Fore.CYAN}There was an error! Please try again.{Style.RESET_ALL}")
            exit();
